...
int
arena_resize(struct arena **arena_inout, size_t new_size)
{
	if (!arena_inout || !*arena_inout)
		return ERR;

	struct arena *a = *arena_inout;

	a = realloc(a, sizeof(*a) + new_size);
	if (!a)
		return ERR;

	if (a->max < new_size) { /* growing? - clear the new area */
		memset(a->data + a->max, 0, new_size - a->max);
	}
...
/* grow buffer by an incremental amount. */
int
arena_increase(struct arena **arena_inout, size_t amount)
{
	if (!arena_inout || !*arena_inout)
		return ERR;
	...
	return arena_resize(arena_inout, a->max + amount);
}

/* access an area of the buffer, returning NULL if out of bounds */
void *
arena_data(struct arena *a, int offset, size_t len)
{
	if (offset + len > a->max)
		return NULL;
